
// This file is maintained for backward compatibility
// It re-exports functionality from the restructured pdf modules
export * from './pdf/core';
export * from './pdf/render';
export * from './pdf/textElement';
export * from './pdf/regionExtraction';
export * from './pdf/textVisualization';
export * from './pdf/batchExtraction';
export * from './pdf/safeBufferUtils';
export * from './pdf/extractionProcessor';
export * from './pdf/documentAnalyzer';
export * from './pdf/extractionUtils';
